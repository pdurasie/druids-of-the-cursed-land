...
object GeometryUtil {
  def generateRandomPointInGeometry(geometry: Geometry): Point = {
    geometry match {
      ...
      case _ =>
        if (geometry.isInstanceOf[Polygon] || geometry
    ...
    LazyList
      .continually {
        ...
        val coordinateSequence = polygon.getFactory.getCoordinateSequenceFactory
          .create(Array(new Coordinate(x, y)))
        new Point(coordinateSequence, polygon.getFactory)
      }
      .find(_.within(polygon))
      .get
  }

  def getIntersectingLine(geohash: String,
                          line: LineString): Option[LineString] = {
    ...
    GeohashUtil.getPolygonFromGeohash(geohash).flatMap { polygon =>
      ...
      if (intersection.equalsExact(line, 0)) {
        ...
        Some(if (intersection.getGeometryType == "MultiLineString") {
          UnaryUnionOp
            .union(intersection.asInstanceOf[MultiLineString])
  ...
  def getIntersectingPolygon(geohash: String,
                             polygon: Polygon): Option[Geometry] = {
